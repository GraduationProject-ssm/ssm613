...
### 点击播放视频 ▼
[![Watch the video](https://i.sstatic.net/Vp2cE.png)](https://www.bilibili.com/video/BV1Tm8QeZE4a?p=13)


# 第一章 绪论
## 1.1 选题背景
目前整个社会发展的速度，严重依赖于互联网，如果没有了互联网的存在，市场可能会一蹶不振，严重影响经济的发展水平，影响人们的生活质量。计算机的发展，不管是从硬件还是软件，都有很多技术储备，每年都有很多的技术和软件产生，纵观各个领域，无一不用互联网软件，办公用的还是电脑居多，但是人手一台智能设备已经变成了现在人们的生活日常，哪怕的普通的老百姓都成了上知天文下知地理的所在，这些都依赖于互联网技术。互联网技术在信息的传播方面是巨大的，而信息的处理就变成了各种产业管理者和维护者的重任。人们已经习惯了互联网的存在，所以经常操作一些互联网产品变成了日常，所以开发一个个性化旅游攻略定制系统，让人们使用是没有一点问题的，并且在这个过程中不仅能够规范个性化旅游攻略信息管理的使用流程还有信息处理流程，也能让整个信息的传播处理，都存在一种可控制的范围，最重要的是，计算机在个性化旅游攻略信息管理方面可以给管理者提供更好的帮助。
## 1.2 选题意义
当开发软件变成了一个潮流之后，会发现不管任何行业都能开发适合自己的软件，不管是内部员工管理，还是财务管理，甚至业务管理都可以数据化，并且可以对这些数据集中处理，进而根据数据处理结果就自然而然的提高了管理水平，最重要的是，开发一个软件投入使用，开发过程其实就是梳理行业痛点的过程，就是让软件来弥补行业的管理漏洞，不断的优化事物的处理流程。个性化旅游攻略定制系统就是一款专门开发的软件，通过Web技术，让使用者可以在任何一台智能设备上面通过浏览器进行操作使用，对数据的处理不在局限于地域距离，只要软件开发到位，使用起来方便，达到预期目的，会发现有软件和没有软件的区别是很大的，有了这款软件之后，会发现数据的存储安全方面，比起之前的满屋寻找记录的优势是多么的巨大。
## 1.3 研究内容
...
### 3.1.2操作可行性
个性化旅游攻略定制系统是根据用户经常使用的页面操作流程来进行设计的，并且页面保证统一，从视觉角度和操作角度上都能达到使用要求。
### 3.1.3经济可行性
在本次开发过程中，因为需要通过电脑来进行配置开发的环境，通过对技术的分析，发现目前正在使用的电脑是可以满足开发需要的，并不需要太多的金钱对电脑进行更换。所以，从经济角度上分析，可以满足开发要求。

从以上三个角度来进行分析论证，证明了个性化旅游攻略定制系统是可以正常开发并且使用的。
## 3.2系统流程分析
...
管理员用例图的绘制结果见图3-5。管理员登录进入本系统操作的功能包括对景点信息，景点项目，旅游路线，用户上传信息，标签分类信息等进行管理。

![](/md/blog.005.png)

图3-5 管理员用例图

用户用例图的绘制结果见图3-6。用户登录进入本系统操作的功能包括对景点，景点项目，旅游路线信息进行评论，收藏，查看等操作，用户可以上传景点信息，对用户上传信息进行管理。
...
管理员功能结构图的绘制结果见图4-1。管理员登录进入本系统操作的功能包括对景点信息，景点项目，旅游路线，用户上传信息，标签分类信息等进行管理。

![](/md/blog.007.png)

图4-1 管理员功能结构图

用户功能结构图的绘制结果见图4-2。用户登录进入本系统操作的功能包括对景点，景点项目，旅游路线信息进行评论，收藏，查看等操作，用户可以上传景点信息，对用户上传信息进行管理。
...
（1）使用Visio这样的常用的实体属性图绘制工具来绘制景点信息实体属性图，绘制结果见图4-3。
...
（3）使用Visio这样的常用的实体属性图绘制工具来绘制旅游路线实体属性图，绘制结果见图4-5。
...
通过上节内容可以知晓数据库中的各个实体，并通过一定方式把这些实体表示的内容进行数据表的转换，通常来说，每个实体都会对应一张具体的数据表，在本系统指定的数据库中创建命名好的数据库，才可以对数据表进行创建与设计。个性化旅游攻略定制系统数据表设计结果展示如下：
...
|id (主键)|主键|bigint(20)|否|
|addtime|创建时间|timestamp|否|
|jingdianmingcheng|景点名称|varchar(200)|是|
|jingdianfenlei|景点分类|varchar(200)|是|
|jingdiandizhi|景点地址|varchar(200)|是|
|xiangmumingcheng|项目名称|varchar(200)|是|
|xiangmujiage|项目价格|float|是|
|xiangmujieshao|项目介绍|longtext|是|
|xiangmutupian|项目图片|varchar(200)|是|
|thumbsupnum|赞|int(11)|是|
|crazilynum|踩|int(11)|是|
表4-7 景点信息表
...
|id (主键)|主键|bigint(20)|否|
|addtime|创建时间|timestamp|否|
|jingdianmingcheng|景点名称|varchar(200)|否|
|jingdianfenlei|景点分类|varchar(200)|否|
|biaoqianfenlei|标签分类|varchar(200)|是|
|jingdiandizhi|景点地址|varchar(200)|是|
|kaifangshijian|开放时间|varchar(200)|是|
|menpiaoleixing|门票类型|varchar(200)|是|
|jingdiandianhua|景点电话|varchar(200)|是|
|jingdiangonglve|景点攻略|longtext|是|
|jingdiantupian|景点图片|varchar(200)|是|
|thumbsupnum|赞|int(11)|是|
|crazilynum|踩|int(11)|是|
...
|id (主键)|主键|bigint(20)|否|
|addtime|创建时间|timestamp|否|
|luxianmingcheng|路线名称|varchar(200)|是|
|biaoqianfenlei|标签分类|varchar(200)|是|
|qidian|起点|varchar(200)|是|
|zhongdian|终点|varchar(200)|是|
|quanchengjuli|全程距离|varchar(200)|是|
|lvxingtianshu|旅行天数|int(11)|是|
|luxianxiangqing|路线详情|longtext|是|
|luxiantupian|路线图片|varchar(200)|是|
|thumbsupnum|赞|int(11)|是|
|crazilynum|踩|int(11)|是|
|clicktime|最近点击时间|datetime|是|
...
|kaifangshijian|开放时间|varchar(200)|是|
|jingdiandianhua|景点电话|varchar(200)|是|
|jingdianjieshao|景点介绍|longtext|是|
|jingdiantupian|景点图片|varchar(200)|是|
...
## 5.1管理员功能实现
### 5.1.1 景点信息管理
管理员进入如图5-1所示的景点信息管理界面之后，管理员点击信息显示栏中最右侧的查看，修改，查看评论，删除按钮可依次完成景点信息的查看，修改，查看景点评论，删除等操作。管理员可以为指定景点添加项目信息。

![](/md/blog.015.png)

图5-1 景点信息管理界面
### 5.1.2 景点项目管理
管理员进入如图5-2所示的景点项目管理界面之后，管理员点击信息显示栏中最右侧的查看，修改，查看评论，删除按钮可依次完成景点项目信息的查看，修改，查看景点项目的评论信息，删除等操作。

![](/md/blog.016.png)

图5-2 景点项目管理界面
### 5.1.3 旅游路线管理
管理员进入如图5-3所示的旅游路线管理界面之后，管理员点击信息显示栏右侧的查看，修改，查看评论，删除按钮可依次完成旅游路线信息的查看，修改，查看旅游路线评论信息，删除等操作。

![](/md/blog.017.png)

图5-3 旅游路线管理界面
### 5.1.4 用户上传管理
管理员进入如图5-4所示的用户上传管理界面之后，管理员点击信息显示栏右侧的审核，查看，删除按钮可依次完成用户上传信息的审核，查看，删除等操作。
...
## 5.2用户功能实现
### 5.2.1 景点信息
用户进入如图5-5所示的景点信息界面之后，用户收藏喜欢的景点，查看景点开放时间，门票类型，景点电话等信息，可以在景点信息界面的下方评论区域发布对景点的评论信息。

![](/md/blog.019.png)

图5-5 景点信息界面
### 5.2.2 景点项目
用户进入如图5-6所示的景点项目界面之后，用户收藏喜欢的景点项目，查看景点项目的名称，景点项目价格，景点地址等信息，可以在景点项目界面的下方评论区域发布对景点项目的评论信息。

![](/md/blog.020.png)

图5-6 景点项目界面
### 5.2.3 旅游路线
用户进入如图5-7所示的旅游路线界面之后，用户收藏喜欢的旅游路线，查看旅游路线的全程距离，旅行天数，起点，终点等信息，可以在旅游路线界面的下方评论区域发布对旅游路线的评论信息。

![](/md/blog.021.png)

图5-7 旅游路线界面
### 5.2.4 用户上传管理
用户进入如图5-8所示的用户上传管理界面之后，用户点击信息显示栏右侧的查看，修改，删除按钮可依次实现对用户上传信息的查看，修改，删除等操作，用户可以添加用户上传信息，用户上传信息包括景点名称，开放时间，景点电话，景点图片等信息。
